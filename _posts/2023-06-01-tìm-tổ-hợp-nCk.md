...
$$A=1 \cdot 2 \cdot 3 \cdot \ldots \cdot (p-1) \cdot p \cdot (p+1) \cdot \ldots \cdot (2p-1) \cdot 2p \cdot (2p+1) \\ \cdot \ldots \cdot (p^2-1) \cdot p^2 \cdot (p^2+1) \cdot \ldots \cdot (n-1) \cdot n$$ 
$$=1 \cdot 2 \cdot 3 \cdot \ldots \cdot (p-1) \cdot (p+1) \cdot \ldots \cdot (2p-1) \cdot (2p+1) \cdot \ldots \\ \cdot (p^2-1) \cdot (p^2+1) \cdot \ldots \cdot (n-1) \cdot n \cdot(p \cdot 2p \cdot \ldots \cdot \lfloor n/p \rfloor p)$$ 
$$=1 \cdot 2 \cdot 3 \cdot \ldots \cdot (p-1) \cdot (p+1) \cdot \ldots \cdot (2p-1) \cdot (2p+1) \cdot \ldots \\ \cdot (p^2-1) \cdot (p^2+1) \cdot \ldots \cdot (n-1) \cdot n \cdot(1 \cdot 2 \cdot \ldots \cdot \lfloor n/p \rfloor ) \cdot p^{\lfloor n/p \rfloor}$$
$$=1 \cdot 2 \cdot 3 \cdot \ldots \cdot (p-1) \cdot (p+1) \cdot \ldots \cdot (2p-1) \cdot (2p+1) \cdot \ldots \\ \cdot (p^2-1) \cdot (p^2+1) \cdot \ldots \cdot (n-1) \cdot n \cdot \lfloor n/p \rfloor! \cdot p^{\lfloor n/p \rfloor}$$

Sau khi loại $p^{\lfloor n/p \rfloor}$ ra khỏi $A$, ta cần tìm giá trị của $$B=1 \cdot 2 \cdot 3 \cdot \ldots \cdot (p-1) \cdot (p+1) \cdot \ldots \cdot (2p-1) \cdot (2p+1) \cdot \ldots \\ \cdot (p^2-1) \cdot (p^2+1) \cdot \ldots \cdot (n-1) \cdot n$$
$$C=\lfloor n/p \rfloor!$$
...
- Với $C$, tương tự như cách tìm $c(n)$, ta gọi đệ quy để phân tích tiếp $\lfloor n/p \rfloor!$ rồi lặp lại bước $(**)$, và chứng minh được số lần gọi đệ quy là $log_pn$.
...
### Sử dụng như nào cho hiệu quả
Đây là bảng so sánh các cách làm
|        | MOD không phải nguyên tố | Độ phức tạp tiền xử lí | Độ phức tạp truy vấn |
|--------|--------------------------|------------------------|----------------------|
| **Cách 2** | x                        | $O(n^2)$                | $O(1)$                 |
| **Cách 3** | x                        | $O(1)$                   | $O(nlogn)$             |
| **Cách 4** |                          | $O(1)$                   | $O(k)$                |
| **Cách 5** |                          | $O(n)$                  | $O(1)$                 |
| **Cách 6** | x                        | $O(M)$ / $O(log_Mn)$         | $O(1)$ / $O(M)$            |
| **Cách 8** | x                        | $O(M)$ / $O((logM)^2)$       | $O(1)$ / $O(M)$            |